...
class Recipe(models.Model):
    ...
    category = models.ForeignKey(
        Category, null=False, on_delete=models.PROTECT, verbose_name="Категория"
    )  # 1 блюдо - 1 категория
    dish_name = models.CharField(max_length=100, verbose_name="Название блюда")
    picture = models.ImageField(
        upload_to="pictures/", blank=False, null=False, verbose_name="Изображение блюда"
    )
    description = models.CharField(
        max_length=500, blank=False, null=False, verbose_name="Описание блюда"
    )
    text = CKEditor5Field(verbose_name="Шаги приготовления")

    notified_saved = models.BooleanField(
        default=False, verbose_name="Уведомление о 500 сохранениях отправлено"
    )
    notified_top = models.BooleanField(
